...
/**
JotForm API - PHP/Laravel Client
 *
 * @copyright   2018 Damilola Ogunmoye, Tedbree Digital.
...
use TedbreeDigital\JotForm\Exceptions\JotFormException;
use TedbreeDigital\JotForm\Traits\DebugTools;
...
class JotForm {
	...
	public function __construct() {
        $this->baseURL = (config('jotform.region') == 'eu')
                        ? 'https://eu-api.jotform.com'
                        : 'https://api.jotform.com';

		$this->apiVersion= config('jotform.version', 'v1');
		$this->apiKey =  config('jotform.key', '');
		$this->debugMode = config('jotform.debug', false);
		$this->outputType = strtolower(config('jotform.output', 'json'));
		$user = $this->getUser();
